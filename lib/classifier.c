...
pof_cls_rule_init(struct cls_rule *rule, const struct match_x *match, int priority)
{
    ...
    VLOG_INFO("++++++++++xyh pof_cls_rule_init: match->wc.masks.value[0][0]=%d match->wc.masks.value[0][7]=%d match->wc.masks.value[0][8]=%d",match->wc.masks.value[0][0],match->wc.masks.value[0][7],match->wc.masks.value[0][8]);
