...
//    	ovs_be32 *pof_flow_value32=pof_flow_u64_value(src, i).be32;
//    	struct be64 pof_flow_value64=pof_flow_u64_value(src, i).be64;
        //if ((pof_flow_u64_value(src, i).be32)||(pof_flow_u64_value(src, i).be64.hi)||(pof_flow_u64_value(src, i).be64.lo))
//    	ovs_be128 dst;
//		dst.be64.hi = htonll(src.u64.hi);
//		dst.be64.lo = htonll(src.u64.lo);
    	//if (pof_flow_u64_value(src, i)&&pof_flow_u64_value(src, i+1))
    	if (pof_flow_u64_value(src, i))
    	{
        	VLOG_INFO("+++++++++++xyh pof_miniflow_map_init:before ovs_be128 ");
            flowmap_set(&flow->map, i, 1);
            VLOG_INFO("+++++++++++xyh pof_miniflow_map_init:after ovs_be128");
        }
    }
}
