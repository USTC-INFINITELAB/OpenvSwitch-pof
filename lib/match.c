...
pof_match_set_field_id(struct match_x *match, unsigned int field_id_idx, ovs_be16 value)
{
    ...
    VLOG_INFO("+++++++++++xyh pof_match_set_field_id: field_id = %d,mask = %d,field_id_idx = %d",match->flow.field_id[field_id_idx],match->wc.masks.field_id[field_id_idx],field_id_idx);
...
pof_match_set_offset(struct match_x *match, unsigned int offset_idx, ovs_be16 value)
{
    ...
    VLOG_INFO("+++++++++++xyh pof_match_set_offset: offset = %d,mask = %d,offset_idx = %d",match->flow.offset[offset_idx],match->wc.masks.offset[offset_idx],offset_idx);
...
pof_match_set_length(struct match_x *match, unsigned int length_idx, ovs_be16 value)
{
    ...
    VLOG_INFO("+++++++++++xyh pof_match_set_length: length = %d,mask = %d,length_idx = %d",match->flow.len[length_idx],match->wc.masks.len[length_idx],length_idx);
...
pof_match_set_value(struct match_x *match, unsigned int value_idx, const struct in6_addr *value)
{
    ...
    for(i=0; i<POF_MAX_FIELD_LENGTH_IN_BYTE;i++){
        match->wc.masks.value[value_idx][i] =255;
        match->flow.value[value_idx][i] = *((uint8_t*)value+i);
        VLOG_INFO("+++++++++++xyh pof_match_set_value_masked: value = %d,mask = %d,value_idx = %d,i=%d",match->flow.value[value_idx][i],match->wc.masks.value[value_idx][i],value_idx,i);
...
{
    /*match_set_reg_masked(match, reg_idx, value, UINT32_MAX);*/
	VLOG_INFO("+++++++++++xyh pof_match_set_value_masked 1 ");
    int i=0;
    ovs_assert(value_idx < POF_N_VALUES);
    for(i=0; i<POF_MAX_FIELD_LENGTH_IN_BYTE;i++){
...
{
    struct miniflow tmp;
    VLOG_INFO("+++++++++++xyh pof_minimatch_init: before pof_miniflow_map_init");
    pof_miniflow_map_init(&tmp, &src->flow);
    VLOG_INFO("+++++++++++xyh pof_minimatch_init: after pof_miniflow_map_init");
    /* Allocate two consecutive miniflows. */
    miniflow_alloc(dst->flows, 2, &tmp);
    pof_miniflow_init(dst->flow, &src->flow);
    VLOG_INFO("+++++++++++xyh pof_minimatch_init: after pof_miniflow_init");
    pof_minimask_init(dst->mask, &src->wc);
    VLOG_INFO("+++++++++++xyh pof_minimatch_init: after pof_minimask_init");
}
