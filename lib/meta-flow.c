...
{
    if (!mask || is_all_ones(mask, mf->n_bytes)) {
    	VLOG_INFO("+++++++++++xyh pof_mf_set: is_all_ones ");
...
//        VLOG_INFO("+++++++++++xyh pof_match_set_value_masked 0 ");
//        value_idx=(unsigned int)(mf->id - MFF_VALUE0);
//        VLOG_INFO("+++++++++++sqy pof_mf_set: value_idx= %d",mf->id - MFF_VALUE0);
//	    const struct in6_addr *valuein;
//	    valuein=&(value->ipv6);
//	    VLOG_INFO("+++++++++++sqy pof_mf_set: valuein");
//	    const struct in6_addr *maskin;
//	    maskin=&(mask->ipv6);
//
...
//                match->wc.masks.value[value_idx][i] =*((uint8_t*) maskin+i);
//                match->flow.value[value_idx][i] = *((uint8_t*)valuein+i);
...
//
//            }
//        VLOG_INFO("+++++++++++sqy pof_mf_set: after MFF_VALUE7");
        break;
    }
