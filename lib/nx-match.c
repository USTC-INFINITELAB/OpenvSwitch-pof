...
{
	VLOG_INFO("+++++++++++xyh is_cookie_pseudoheader: header =%d",header);
...
{
	VLOG_INFO("+++++++++++xyh nx_pull_header__: b->size =%ld",b->size);
    if (b->size < 4) {
        goto bad_len;
    }
    ...
    *header = ((uint64_t) ntohl(get_unaligned_be32(b->data))) << 32;
    ...
    VLOG_INFO("+++++++++++xyh nx_pull_header__: nxm_length(*header)=%d,nxm_experimenter_len(*header)=%d,nxm_header_len(*header)=%d",nxm_length(*header),nxm_experimenter_len(*header),nxm_header_len(*header));
    ofpbuf_pull(b, nxm_header_len(*header));

    if (field) {
        *field = mf_from_oxm_header(*header);
        if (!*field && !(allow_cookie && is_cookie_pseudoheader(*header))) {
        	VLOG_INFO("+++++++++++xyh nx_pull_header__: OFPERR_OFPBMC_BAD_FIELD,header =%ld,allow_cookie=%d",*header,allow_cookie);
                ...
                union mf_value *value, union mf_value *mask)
{
    ...
    int width;
    VLOG_INFO("+++++++++++xyh nx_pull_entry__: nx_pull_header__");
    header_error = nx_pull_header__(b, allow_cookie, header, &field);
    if (header_error && header_error != OFPERR_OFPBMC_BAD_FIELD) {
    	VLOG_INFO("+++++++++++xyh nx_pull_entry__: header_error ");
        return header_error;
    }

    payload_len = nxm_payload_len(*header);
    payload = ofpbuf_try_pull(b, payload_len);
    VLOG_INFO("+++++++++++xyh nx_pull_entry__: payload=%d ,payload_len=%d",payload,payload_len);
    if (!payload) {
    	VLOG_INFO("+++++++++++xyh nx_pull_entry__: OFPERR_OFPBMC_BAD_LEN");
    ...
    if (nxm_hasmask(*header)
    	...
    	VLOG_INFO("+++++++++++xyh nx_pull_entry__: OFPERR_OFPBMC_BAD_WILDCARDS");
    ...
    if (mask) {
    	...
    	VLOG_INFO("+++++++++++xyh nx_pull_entry__: OFPERR_OFPBMC_BAD_MASK");
            ...
            const struct tun_table *tun_table)
{
    ...
    while (b.size) {
        ...
        error = nx_pull_match_entry(&b, cookie != NULL, &field, &value, &mask);
        VLOG_INFO("+++++++++++sqy nx_pull_pof_raw: nx_pull_match_entry ");
        if (error) {
           ...
           ovs_be64 cookie, ovs_be64 cookie_mask)
{
    ...
    const size_t start_len = b->size;
    VLOG_INFO("++++++++++xyh nx_put_pof_raw: start_len = %d ",start_len);
    int match_len;
    int i;
    ...
    for (i = 0; i < POF_N_FIELD_IDS; i++) {
        ...
        VLOG_INFO("++++++++++xyh nx_put_pof_raw: MFF_FIELD_ID0b->size = %d flow->field_id[i]=%d i = %d",b->size,flow->field_id[i],i);
        nxm_put_16m(b, MFF_OFFSET0 + i, oxm, flow->offset[i],
                    match->wc.masks.offset[i]);
        VLOG_INFO("++++++++++xyh nx_put_pof_raw: MFF_OFFSET0b->size = %d flow->offset[i]=%d",b->size,flow->offset[i]);
        nxm_put_16m(b, MFF_LENGTH0 + i, oxm, flow->len[i],
                    match->wc.masks.len[i]);
        VLOG_INFO("++++++++++xyh nx_put_pof_raw: MFF_LENGTH0b->size = %d flow->len[i]=%d",b->size,flow->len[i]);
        nxm_put(b, MFF_VALUE0 + i, oxm, flow->value[i], match->wc.masks.value[i],
                sizeof flow->value[i]);
        VLOG_INFO("++++++++++xyh nx_put_pof_raw: MFF_VALUE0b->size = %d flow->value[i][0]=%d match->wc.masks.value[i][0]=%d,sizeof flow->value[i]=%d",b->size,flow->value[i][0],match->wc.masks.value[i][0],sizeof flow->value[i]);
    }
    /* Cookie. */
    if (cookie_mask) {
        bool masked = cookie_mask != OVS_BE64_MAX;
...
nxm_field_by_header(uint64_t header)
{
    ...
    VLOG_INFO("+++++++++++xyh nxm_field_by_header: header_no_len=%ld",header_no_len);
    HMAP_FOR_EACH_IN_BUCKET (nfi, header_node, hash_uint64(header_no_len),
        ...
        if (header_no_len == nxm_no_len(nfi->nf.header)) {
            if (nxm_length(header) == nxm_length(nfi->nf.header) ||
            	...
            	//VLOG_INFO("+++++++++++xyh nxm_field_by_header: nfi->nf.name=%s",*(nfi->nf.name));
                return &nfi->nf;
            }
            else {
            	VLOG_INFO("+++++++++++xyh nxm_field_by_header: do not match 1");
                return NULL;
            }
        }
    }
    VLOG_INFO("+++++++++++xyh nxm_field_by_header: do not match 2");
    return NULL;
}
