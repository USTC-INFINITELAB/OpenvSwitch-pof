...
vconn_send(struct vconn *vconn, struct ofpbuf *msg)
{
    ...
    if (!retval) {
    	VLOG_INFO("+++++++++++xyh vconn_send:do send");
...
vconn_send_block(struct vconn *vconn, struct ofpbuf *msg)
{
    ...
    fatal_signal_run();
    VLOG_INFO("+++++++++++xyh vconn_send_block");
                       ...
                       struct ofpbuf **replyp)
{
    ...
    *replyp = NULL;
    ...
    /* Send request. */
    VLOG_INFO("+++++++++++xyh vconn_transact_noreply: vconn_send_block");
    request_xid = ((struct ofp_header *) request->data)->xid;
    error = vconn_send_block(vconn, request);
    VLOG_INFO("+++++++++++xyh vconn_transact_noreply: vconn_send_block %d",error);
    if (error) {
        ofpbuf_delete(request);
        return error;
    }
    ...
    VLOG_INFO("+++++++++++xyh vconn_transact_noreply: vconn_send_barrier %d",error);
    if (error) {
        return error;
    }
    ...
    for (;;) {
        ...
        VLOG_INFO("+++++++++++xyh vconn_transact_noreply: vconn_recv_block %d",error);
        if (error) {
        ...
        VLOG_INFO("+++++++++++xyh vconn_transact_noreply: msg_xid=%d,request_xid=%d",msg_xid,request_xid);
                                ...
                                struct ofpbuf **replyp)
{
    ...
    LIST_FOR_EACH_POP (request, list_node, requests) {
        ...
        VLOG_INFO("+++++++++++xyh vconn_transact_multiple_noreply: vconn_transact_noreply %d",error);
        if (error || *replyp) {
